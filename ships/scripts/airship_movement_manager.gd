...
func _physics_process(delta: float) -> void:
	apply_central_force(global_basis.x * acceleration * strafe_input * 0.2)
	apply_central_force(global_basis.y * acceleration * lift_input * 0.2)
	...
	var veldiff : Vector3 = Vector3(
		max(0, get_directional_speed_ratio(global_basis.x, airship.actor_data.speed * 0.2) - strafe_input),
		max(0, get_directional_speed_ratio(global_basis.y, airship.actor_data.speed * 0.2) - lift_input),
	...
	#print(get_directional_speed_ratio())
